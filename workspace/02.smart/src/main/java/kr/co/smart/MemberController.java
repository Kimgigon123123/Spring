...
//	      -H "Authorization: Bearer AAAAPIuf0L+"
	...
	public String kakaoCallback(String code, String state, HttpSession session,Model model) {
		...
		// &client_id=f590b6c3a2f0f101c550477871a90924
		// &client_secret=40yyY0SWvYBLgmezo3BVOu19wrOdCmso
		// &code=
...
//		 -d "code=${AUTHORIZE_CODE}"
		...
		// DB에  카카오 정보 저장하기 - 존재여부를 확인하여 신규/변경 저장
		if (service.member_info(vo.getUserid()) == null) {
			service.member_join(vo);
		} else {
			service.member_update(vo);
...
//			return "redirect:/";
	...
	private String redirectURL(HttpSession session,Model model) {
		if(session.getAttribute("redirect")==null) {
			...
			session.removeAttribute("redirect"); // redirect에 필요한 정보 사용후 세션에서 삭제
